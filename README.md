...
锁屏时释放增强修饰键，解锁／唤醒后延迟重建输入连接；Hyper／Meh 的暂时性恢复失败会最多自动重试五次。等待按键松开后再恢复，外部映射冲突或权限撤销时保持暂停。
输入处理前核对触发键的实际按下状态；若锁屏期间丢失松键事件，丢弃残留状态，不补发单击映射，避免普通点击或按键持续带上 Hyper／Meh 修饰键。
