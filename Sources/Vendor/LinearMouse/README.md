...
MacTools' configurable modifier adapter also follows the upstream `ModifierActionsTransformer` approach (v0.11.4): transform axes/speed before smoothing and clear handled modifier flags; zoom emits Command+keypad plus/minus. The upstream modifier/UI implementation is not copied. MacTools passes multiple modifiers through unchanged, protects already-horizontal Shift input, and restores physical flags after zoom.
